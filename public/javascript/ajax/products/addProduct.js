...
document.getElementById('cardImageUpload').addEventListener('change', (event) => {
...
document.getElementById('productImagesUpload').addEventListener('change', (event) => {
    event.preventDefault()
    Array.from(event.target.files).forEach((file) => {
        if (uploadedFiles.length < 6) { // Limit the number of uploaded images if needed
            uploadedFiles.push(file);
            const reader = new FileReader();

            reader.onload = (e) => {
                const imgWrapper = document.createElement('div');
                imgWrapper.classList.add('relative', 'h-[178px]', 'w-[146px]', 'rounded-[10px]', 'border', 'border-[#D8DEE6]');
